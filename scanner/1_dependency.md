...
<span class="specific-title">실행 방법 및 결과</span>
1. package.json이 존재하는 디렉토리에서 다음 명령어를 실행합니다.  
   ...
   ```
2. 결과  
  - 동일 디렉토리 내에 'fosslight_report_dep_[datetime].xlsx' 결과 파일이 생성된 것을 확인합니다.  
...
3. 결과
  - build/reports/license/dependency-license.json 예시
   ...
   ```  
  - 동일 디렉토리 내에 'fosslight_report_dep_[datetime].xlsx' 결과 파일 
...
3. 결과  
  - target/generated-resources/licenses.xml 예시   
...
1. Linux  
  - Podfile과 Podfile.lock만 있으면 간단한 환경 설정 후 CocoaPods dependency를 분석할 수 있습니다.  
    1. Ubuntu에서 gem package manager를 이용하기 위해 Ruby version 2.0 이상, ubuntu-dev-tools 설치  
    ```
    # 먼저 ruby가 설치되었는지 확인합니다.
    $ ruby -v
    ...
    ```
    3. spec repository 다운로드하고 pod package를 설치  
    ```
    $ pod setup
...
1. Linux/MacOS
  ...
  ```  
2. Windows  
  1).[FOSSLight Dependency Scanner Releases](https://github.com/fosslight/fosslight_dependency_scanner/releases) > Assets > fosslight_dependency_windows.zip 다운로드 후 압축을 해제합니다.  
  2).fosslight_dependency.exe를 해당 path에 위치시킨 후, 더블 클릭하여 실행합니다.   
